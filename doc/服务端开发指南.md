...
+   服务器文件存储位置只配置一个根目录 `FILE_PATH`：博客、照片墙图片、友链内容与 README 目录（`$FILE_PATH/blogs`、`$FILE_PATH/photo_wall`、`$FILE_PATH/friend_links`、`$FILE_PATH/README`）由服务端启动时创建并校验；README 内容在每次执行 `tools/rebuild.sh` 的前端构建前由 `tools/pull-readme.sh` 拉取。
...
+   构建生成版本，生成 `dist` 目录（目录生成位置在环境变量中配置）
    ...
    > `npm run build` 时 `client/vite.config.ts` 会直接读取 `$FILE_PATH/README/README.md`，并把内容注入《关于我》页面。
