# 服务端开发指南

项目开发和生产环境使用 `Ubuntu-24.04` 和 `Ubuntu-26.04`。

>   说明：以下出现所有 `${NAME}` 格式的变量为用户的**个性化配置**，而非环境变量。

## 克隆仓库

```bash
# 使用 SSH 克隆是为了使用 GitHub Action 自动化拉取新更改并重新部署
git clone git@github.com:${USER_NAME}/${REPO_NAME}.git ${PROJECT_PATH}
cd ${PROJECT_PATH}
```

## 项目配置

项目配置文件位于 `${PROJECT_PATH}/conf` 目录下。各配置条目修改后需重启后端服务以应用。

+   复制 `.env.example` 为 `.env` 并填写环境变量。
+   `cache.yml` 为 Redis 中各类缓存过期时间配置。
+   `page_size.yml` 为各筛选器分页每页条数配置。
+   服务器文件存储位置只配置一个根目录 `FILE_PATH`：博客、照片墙图片、友链内容与 README 目录（`$FILE_PATH/blogs`、`$FILE_PATH/photo_wall`、`$FILE_PATH/friend_links`、`$FILE_PATH/README`）由服务端启动时创建并校验；README 内容与友链数据仓库分别在每次执行 `tools/rebuild.sh` 的前端构建前由 `tools/pull-readme.sh`、`tools/pull-friend-links.sh` 拉取。

## 前端

本条目下操作默认在 `${PROJECT_PATH}/client` 目录下执行。

+   安装 `Node.js` 和 `npm`

    ```bash
    curl -fsSL https://deb.nodesource.com/setup_22.x | bash -
    apt install -y nodejs
    ```

+   安装 `npm` 项目依赖

    ```bash
    npm install
    ```

+   构建生成版本，生成 `dist` 目录（目录生成位置在环境变量中配置）

    ```bash
    npm run build
    ```

    > `npm run build` 时 `client/vite.config.ts` 会直接读取 `$FILE_PATH/README/README.md` 注入《关于我》页面，并读取 `$FILE_PATH/friend_links/meta.yaml` 生成 `virtual:friend-links` 注入《友情链接》页面；对应头像交由 Vite 资源管线按内容哈希命名并复制进产物目录。

    >   [!Warning]
    >
    >   友链仓库内容约定：
    >
    >   仓库根目录放置 `meta.yaml` 及若干图片文件。其中 `meta.yaml` 格式要求每个条目包含字段 `id` / `title` / `url` / `description`，其中每条友链对应的图片文件命名为 `${id}.*$`。

## Nginx

+   安装 `nginx`

    ```bash
    apt install nginx
    ```

+   新建站点文件配置并编辑

    ```bash
    nano /etc/nginx/sites-available/${PROJECT_NAME}
    ```

+   启用站点配置

    ```bash
    ln -s /etc/nginx/sites-available/${PROJECT_NAME} /etc/nginx/ sites-enabled/
    nginx -t && systemctl reload nginx
    ```

## 数据库

本条目下操作默认在 `${PROJECT_PATH}/sql` 目录下执行。

+   安装 `postgresql`

    ```bash
    apt install postgresql
    ```

+   创建数据库和用户并设置权限

    ```bash
    sudo -u postgres psql
    ```

    ```postgresql
    CREATE USER ${PGUSER} WITH ENCRYPTED PASSWORD '${PGPASSWORD}';
    CREATE DATABASE ${PGDATABASE} OWNER ${PGUSER};
    GRANT ALL PRIVILEGES ON DATABASE ${PGDATABASE} TO ${PGUSER};
    \q
    ```

+   创建所需数据库表

    ```bash
    psql -f ./create_users.sql       # 角色表 + 权限表 + 角色-权限关联表 + session 表
    psql -f ./create_blogs.sql       # 博客表 + 分类表 + 标签表 + 博客-标签关联表
    psql -f ./create_images.sql      # 图片表
    ```
    
+   配置 PostgreSQL 全局超时时间

    ```bash
    nano /etc/postgresql/*/main/postgresql.conf
    
    statement_timeout = '20s'
    ```
    

## Redis 缓存

公开 GET 接口通过 Redis 做 cache-aside 缓存，降低 PostgreSQL 查询压力；与之内容相关联的写接口成功后自动失效相关缓存。

各项接口的缓存有效期配置在 `conf/cache.yml`（单位：秒）。

+   安装 `redis-server`

    ```bash
    apt install redis-server
    systemctl enable --now redis-server
    ```

## 后端

本条目下操作默认在 `${PROJECT_PATH}/server` 目录下执行。

+   安装 `CMake`

    ```bash
    apt install cmake
    ```

+   安装 `vcpkg` 并设置环境变量

    ```bash
    git clone https://github.com/Microsoft/vcpkg.git ${VCPKG_ROOT}
    ${VCPKG_ROOT}/bootstrap-vcpkg.sh
    
    echo 'export VCPKG_ROOT=${VCPKG_ROOT}' >> ~/.bashrc
    echo 'export PATH="$PATH:$VCPKG_ROOT"' >> ~/.bashrc
    source ~/.bashrc
    ```

+   安装 `pkg-config`

    ```bash
    apt install pkg-config
    ```

+   使用 `vcpkg` 安装所需的库及库运行所需的包

    ```bash
    apt install bison flex autoconf autoconf-archive automake libtool
    
    vcpkg install cpp-httplib:x64-linux
    vcpkg install libpqxx:x64-linux
    vcpkg install libsodium:x64-linux
    vcpkg install nlohmann-json:x64-linux
    vcpkg install redis-plus-plus:x64-linux
    vcpkg install spdlog
    vcpkg install yaml-cpp:x64-linux
    ```

+  拉取 `libcpp-pg-pool`（连接池库，纯头文件，基于 `libpqxx`）

    ```bash
    git clone --depth 1 https://github.com/leventkaragol/libcpp-pg-pool ./third_party/libcpp-pg-pool
    ```

+   构建并后台运行 `C++` 应用程序

    ```bash
    cmake -B build --preset default  # 开发构建
    cmake -B build --preset release  # 发布构建（继承 default，启用 Release 优化）
    
    cmake --build build
    
    ./build/server               # 开发环境临时运行
    ../tools/server-run.sh start # 生产环境启动后端后台运行
    ../tools/server-run.sh stop  # 生产环境终止后端后台运行
    ```

## 项目配置

默认管理员账号由 `sql/create_users.sql` 自动创建：

- 用户名：`root`
- 密码：`root`

root 的密钥已停用（因固定盐需自行设定），仅支持密码登录；可在客户端登录后在「后台管理/用户管理」页面自行设置密钥并启用。

## CI/CD（可选）

本条目下若代码块开头无 `cd` 指令，默认在 `${PROJECT_PATH}/tools` 目录下执行。

### 博客文件远程冷备

+   博客保存目录连接远程仓库并创建 `auto` 分支

    ```bash
    cd ${FILE_PATH}/blogs
    git init
    git remote add origin ${REPO_URL}
    git checkout -b auto
    git commit --allow-empty -m "initial empty commit"
    git push -u origin auto
    ```

+   设置定时同步（每分钟一次）

    ```bash
    chmod +x ./auto-sync-blogs.sh
    
    crontab -e
    * * * * * /bin/bash ${PROJECT_PATH}/tools/auto-sync-blogs.sh >> ${PROJECT_PATH}/tools/auto-sync-blogs.log 2>&1
    ```

### GitHub Actions 自动集成

+   Github 仓库环境变量
    
    在 GitHub 仓库「Settings / Environments」设置部署环境名为 `production`。
    
    在 GitHub 仓库「Settings / Secrets and variables / Actions」中配置环境变量：
    
    | 名称                | 类型     | 说明                                                   |
    | ------------------- | -------- | ------------------------------------------------------ |
    | `DEPLOY_ENABLED`    | Variable | 设为 `true` 开启自动部署；未设置或为其它值时不执行部署 |
    | `DEPLOY_HEALTH_URL` | Variable | 部署完成后的健康检查地址，例如生产环境域名             |
    | `DEPLOY_HOST`       | Secret   | 服务器 IP 或域名                                       |
    | `DEPLOY_PORT`       | Secret   | SSH 端口                                               |
    | `DEPLOY_USER`       | Secret   | SSH 登录用户名                                         |
    | `DEPLOY_SSH_KEY`    | Secret   | SSH 私钥                                               |
    | `DEPLOY_PATH`       | Secret   | 服务器上项目根目录路径                                 |
    
+   `ci.yml` — 持续集成，push 或提交 PR 到 `main` 分支时自动执行

    - 前端：`npm ci` → `npm run type-check` → `npm run build`
    - 后端：安装 vcpkg 依赖（含缓存）→ 以 `release` 预设配置并构建 CMake
    - 冒烟测试：调用测试脚本，启动 PostgreSQL 与 Redis 容器、创建数据表，运行服务端并验证公开 API 及缓存写入/失效

+   `deploy.yml` — 持续部署，merge 到 `main` 分支并成功执行 `ci.yml` 后自动执行。

    - `main` 分支 CI 通过后自动执行，也可在 Actions 页面手动触发（`workflow_dispatch`）
    - 通过 SSH 在服务器上执行 `tools/rebuild.sh` 完成项目重建流程

## 开发测试

测试脚本存放目录为 `${PROJECT_PATH}/test`。

+   后端冒烟测试

    后端冒烟测试脚本为 `smoke-test.sh`，CI 与本地共用：

    脚本默认使用后端可执行文件路径为 `server/build/server`，CI 或自定义路径时用 `SERVER_BIN` 覆盖，如 `SERVER_BIN=${server-build}/server bash test/smoke-test.sh`。

    需要 `psql`、`redis-cli`、`curl` 以及可用的 PostgreSQL 与 Redis。

    脚本使用 `conf/.env`（CI 会在调用前从环境变量生成，本地用现有文件）；数据库表已存在时跳过初始化，端口上已有旧服务端时会先终止、确保测试运行在脚本创建的临时服务端上（测试结束即清理，原服务不会自动恢复）。脚本验证图片、博客、分类、标签四个公开 GET 接口均返回 200，并检查对应 Redis 缓存键已写入；另用一次查不到结果的博客搜索验证空结果不写缓存。
