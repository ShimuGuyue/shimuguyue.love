...
# ============================================================
# 用途：检测 $FILE_PATH/blogs/ 下 .md 文件变更，自动 git add / commit / push
# 部署：由 crontab 或 systemd timer 定时每分钟调用一次
#
# 前置条件：
#   1. $FILE_PATH/blogs 是一个已初始化的 git 仓库，含远程仓库配置
#   2. 本脚本需要有对 $FILE_PATH/blogs 的写权限
#   3. git 已配置 user.name 和 user.email（用于 auto-commit）
#
# 环境变量：
#   FILE_PATH         — 文件根目录，博客内容根目录为 $FILE_PATH/blogs
#   SYNC_REMOTE       — 推送到的远程仓库名（默认 origin）
#   SYNC_BRANCH       — 推送到的分支名（默认 auto；本地不在该分支时自动切换）
#
# 工作原理：
#   $FILE_PATH/blogs 是独立的 git 仓库。
