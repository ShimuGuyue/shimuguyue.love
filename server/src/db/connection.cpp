...
{
    spdlog::info("正在检查项目所需数据库表...");

    const std::vector<std::string> required_tables = {
        "users", "permissions", "user_permissions", "sessions",
    ...
    for (const auto& table_name : required_tables)
    {
        ...
        {
            spdlog::info("数据库表 {} 存在。", table_name);
        }
    }
    // 数据库表额外检查
    // profile 第一条数据存在
